...
if [[ $# -ne 1 ]]; then
  ...
  echo "  Submits one SLURM tuning job per base MLP-style KAN model" >&2
  echo "  (efficientkan, fastkan, fasterkan, wavkan) on the weak_lensing" >&2
  echo "  dataset, each sweeping over the dimension-reduction method" >&2
...
EXPERIMENT="$1"
MODELS=(efficientkan fastkan fasterkan wavkan)
